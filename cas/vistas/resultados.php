...
<h2 class="text-xl font-bold mb-4">
    Gestionar Resultados - Convocatoria <?php echo $convocatoria['numero']; ?>/<?php echo $convocatoria['anio']; ?>
</h2>
<p class="mb-6 text-gray-600">
    <?php echo $convocatoria['puesto']; ?>
</p>

<!-- Listado de resultados existentes -->
<div class="mb-6">
    <h3 class="text-lg font-semibold mb-3">Resultados Actuales</h3>
    
    <?php if (empty($convocatoria['resultados'])): ?>
        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
            <p class="text-yellow-800">
                No hay resultados registrados para esta convocatoria.
            </p>
        </div>
    <?php else: ?>
        <div class="overflow-x-auto rounded-lg shadow">
            <table class="min-w-full bg-white">
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php foreach ($convocatoria['resultados'] as $indice => $resultado): ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-900">
                                <?php echo $resultado['titulo']; ?>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                <a href="<?php echo $resultado['url']; ?>" target="_blank" class="text-blue-600 hover:underline">
                                    <?php echo substr($resultado['url'], 0, 50) . (strlen($resultado['url']) > 50 ? '...' : ''); ?>
                                </a>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <form method="post" action="" class="inline" 
                                      onsubmit="return confirm('¿Está seguro de que desea eliminar este elemento?');">
                                    <input type="hidden" name="accion" value="eliminar_resultado">
                                    <input type="hidden" name="convocatoria_id" value="<?php echo $convocatoria['id']; ?>">
                                    <input type="hidden" name="indice" value="<?php echo $indice; ?>">
                                    <button type="submit" class="text-red-600 hover:text-red-900" title="Eliminar resultado">
...
<div class="bg-white p-6 rounded-lg shadow-md">
    <h3 class="text-lg font-semibold mb-4">Agregar Nuevo Resultado</h3>
    
    <form method="post" action="">
        <input type="hidden" name="accion" value="agregar_resultado">
        <input type="hidden" name="convocatoria_id" value="<?php echo $convocatoria['id']; ?>">
        ...
        <div class="grid grid-cols-1 gap-6">
            <div>
                ...
                <input type="text" id="titulo" name="titulo" required
                       ...
                       placeholder="Ej. RESULTADO DE EVALUACIÓN PRELIMINAR">
            </div>
